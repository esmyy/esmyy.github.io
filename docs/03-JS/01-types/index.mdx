...
转换为 Number 类型的场景，主要是从 String 转换为 Number，有 parseInt、parseFloat、Number 和 `+` 操作符等方式。
...
作为构造函数调用时返回一个对象，这个对象称作包装对象，这个过程称做装箱。
...
我们说到“包装对象”，更多是在解释访问原始类型变量的属性或方法时发生的”包装“过程
...
默认情况下，根据 hint 的不同，会按照以上示意的顺序尝试进行调用

- 如果得到一个原始值，则结束调用路径，返回
...
:::tip 小结
拆箱转换过程中，hint 只影响了调用顺序，与实际返回值的类型无关。
如果要判断拆箱转换的结果，关键是要确定，在不同的情况下 hint 是什么。
