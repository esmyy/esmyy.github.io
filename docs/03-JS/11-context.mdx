...
执行上下文是 JavaScript 执行一段代码时的运行环境。这个环境是什么呢？根据 [ECMA 规范中的定义](https://262.ecma-international.org/5.1/#sec-10.3)，具体来说是三个部分组成
...
:::tip 🤔 由于整个变量提升是上下文级别的，整个上下文级别只有一个变量环境，因此变量环境也可以称作变量对象。
:::
